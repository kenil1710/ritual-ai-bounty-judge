...
/**
 * Epoch milliseconds -> local date string. Ritual Chain's block.timestamp is in
 * MILLISECONDS, so the contract stores deadlines in ms and we render them as-is
 * (no ×1000).
 */
export function formatTimestamp(epochMs?: bigint | number): string {
...
/** Compact "in 2h 5m" / "3m ago" relative label. `epochMs` is in milliseconds. */
export function formatRelative(epochMs?: bigint | number): string {
  if (epochMs === undefined) return "";
